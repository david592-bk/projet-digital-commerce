...
function HomePage() {
  ...
  const submitOrder = async (event) => {
    ...
    try {
      ...
      setMessage(
        "Commande enregistrée avec succès. Un livreur pourra la prendre en charge.",
      );
      setSelectedProduct(null);
      setSelectedMerchant(null);
    } catch (error) {
      setMessage(
        "Impossible de créer la commande. Vérifiez votre connexion ou vos informations.",
      );
    }
  };
  ...
  return (
    <div className="bg-slate-50">
      <div className="mx-auto max-w-7xl space-y-10 px-4 py-8 sm:px-6 lg:px-8">
        {/* Hero section – mixte acheteurs/vendeurs */}
          ...
          <div className="mx-auto flex max-w-6xl flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
            <div className="max-w-2xl">
              ...
              <p className="mt-5 text-lg leading-8 text-slate-600">
                Découvrez les produits des entreprises locales, passez commande
                en quelques clics, ou créez votre boutique pour développer votre
                activité.
              </p>
              ...
              <p className="mt-6 max-w-xl text-sm text-slate-500">
                {currentUser
                  ? "Bienvenue ! Vous pouvez acheter ou créer votre boutique depuis votre tableau de bord."
                  : "Inscrivez-vous gratuitement pour acheter ou lancer votre boutique en ligne."}
              </p>
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-[1.5rem] bg-slate-50 p-6 shadow-sm">
                ...
                <p className="mt-3 text-sm leading-6 text-slate-600">
                  Parcourez les produits des commerçants de votre région, passez
              ...
              <div className="rounded-[1.5rem] bg-slate-50 p-6 shadow-sm">
                ...
                <p className="mt-3 text-sm leading-6 text-slate-600">
                  Lancez votre boutique en ligne, gérez vos produits, commandes
                  et clients depuis un tableau de bord.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h2 className="mt-3 text-3xl font-semibold text-slate-900">
                Produits phares de nos entreprises locales
              </h2>
            </div>
          ...
          {carouselItems.length > 0 ? (
            <div className="mt-8 grid gap-6 lg:grid-cols-[0.9fr_0.7fr] lg:items-center">
              ...
              <div className="space-y-5 rounded-[1.75rem] bg-slate-100 p-8 shadow-sm">
                <div>
                  <p className="text-sm uppercase tracking-[0.2em] text-slate-500">
                    {carouselItems[activeSlide].shop_name}
                  </p>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="rounded-[1.5rem] bg-white p-5">
                    ...
                    <p className="mt-2 text-2xl font-semibold text-slate-900">
                      {carouselItems[activeSlide].category || "Autre"}
                    </p>
                  </div>
                </div>
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <button
                    type="button"
                    onClick={() => selectProduct(carouselItems[activeSlide])}
                    className="rounded-full bg-sky-600 px-6 py-3 text-base font-semibold text-white shadow-sm transition hover:bg-sky-700"
                  >
                    Commander
                  </button>
                  <p className="text-sm text-slate-500">
                    Cliquez sur l’image pour afficher les coordonnées de
                    l’entreprise.
                  </p>
                </div>
              </div>
        ...
        {/* Bloc principal : recherche produits + infos vendeurs */}
        <section className="grid gap-6 lg:grid-cols-[2fr_1fr]">
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm text-slate-500">
                  Tapez un produit ou une boutique pour trouver rapidement ce
          ...
          {/* Panneau d'information vendeur + actions */}
          <aside className="space-y-6 rounded-[2rem] bg-slate-950 p-8 text-white shadow-xl">
            <div className="rounded-[1.75rem] bg-slate-900 p-6">
              ...
              <p className="mt-3 text-sm leading-6 text-slate-300">
                Transformez votre activité avec une vitrine professionnelle, un
                tableau de bord intuitif et un accompagnement simple.
              </p>
              {!currentUser ? (
                <Link
                  to="/become-seller"
                  className="mt-5 inline-block rounded-full bg-white px-5 py-3 text-center text-sm font-semibold text-slate-900 hover:bg-slate-100"
                >
                  Devenir vendeur
                </Link>
              ) : (
                <Link
                  ...
                  className="mt-5 inline-block rounded-full bg-sky-600 px-5 py-3 text-center text-sm font-semibold text-white hover:bg-sky-700"
                >
                  Créer ma boutique
                </Link>
              )}
            ...
            <div className="rounded-[1.75rem] bg-slate-900 p-6">
              ...
              <ul className="mt-4 space-y-3 text-sm text-slate-300">
                ...
                <li>✅ Mise en avant de vos produits</li>
        ...
        {/* Formulaire de commande (affiché quand un produit est sélectionné) */}
        {selectedProduct && (
          <section className="rounded-[2rem] bg-white p-8 shadow-xl">
                ...
                <div className="flex items-center gap-3">
                  <div className="rounded-3xl bg-slate-100 p-4">
                    {selectedProduct.photo_url ? (
                      ...
                      <div className="flex h-28 w-28 items-center justify-center rounded-3xl bg-slate-200 text-slate-500">
                        Image
                      </div>
                    )}
                  </div>
              ...
              <div className="space-y-6 rounded-[1.75rem] bg-slate-950 p-8 text-white shadow-sm">
                <div className="rounded-[1.5rem] bg-slate-900 p-6">
                  ...
                  <p className="mt-3 text-lg font-semibold text-white">
                    {selectedMerchant?.name || selectedProduct.shop_name}
                  </p>
                  <p className="mt-2 text-sm text-slate-300">
                    {selectedMerchant?.address || "Adresse non disponible"}
                  </p>
                  ...
                  {!currentUser ? (
                    <>
                      ...
                      <div className="mt-5 grid gap-3">
                        <Link
                          className="rounded-full bg-white px-5 py-3 text-center text-sm font-semibold text-slate-900"
                          to="/auth/register"
                        >
                          Créer un compte
                        </Link>
                    ...
                    <form onSubmit={submitOrder} className="mt-4 space-y-4">
                      <div>
                        ...
                        <input
                          ...
                          className="mt-1 w-full rounded-full border border-white/20 bg-white/10 px-4 py-2 text-white"
                        />
                      </div>
                      <div>
                        <label className="block text-sm text-slate-300">
                      ...
                      <button
                        type="submit"
                        className="w-full rounded-full bg-sky-600 py-2 font-semibold hover:bg-sky-500"
                      >
        ...
        {/* Section pour encourager la création de boutique */}
        <section className="rounded-[2rem] bg-gradient-to-r from-sky-50 to-white p-8 text-center shadow-md">
          <h2 className="text-2xl font-bold text-slate-900">
            Vous gérez une entreprise ?
          </h2>
          <p className="mx-auto mt-2 max-w-xl text-slate-600">
            Rejoignez Kuhuza-Digital et offrez à vos clients une expérience
            d’achat moderne.
          </p>
          <div className="mt-6">
            {!currentUser ? (
              <Link
                to="/become-seller"
                className="rounded-full bg-sky-600 px-8 py-3 text-base font-semibold text-white shadow-sm hover:bg-sky-700"
              >
